...
Template w którym pracujemy uwzglednia już ekran Welcome Screen, ekrany logowania, rejestracji, profilu oraz ustawień więc nie musisz tego dodatkowo opisywać. W `Feature screens` opisz tylko ekrany specyficzne dla tego pomysłu aplikacji.
...
2. Po utworzeniu pliku, zaleć, aby user go przeczytał i zaakceptował. Jeżeli ma jakieś uwagi, popraw je.

3. Gdy użytkownik zaakceptuje plik `docs/IDEA.md` - zacommituj go do repository.

Gdy odpowie "next", przejdź do wykonania polecenia zawartego w `docs/commands/01_start/07_start-app-bundle-id.md`.
